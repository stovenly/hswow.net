...
import { buildInterior, HOUSE_STYLE, type InteriorStyle } from '@engine/world/interior';
import { markCollidable } from '@engine/player/Collider';
import { PALETTE } from '@engine/art/palette';
import { COTTAGE, WORKSHOP, STORE } from './countryside-homes';
...
/**
 * The geometry of the three countryside homes, in its own chunk.
 *
 * Split from `countryside-homes.ts` under ZONE-LOADING.md Phase D: the twenty
 * builders below are most of what these rooms cost to *download*, and none of
 * them is needed until somebody opens one of the three doors. What stays in the
 * definition file is what the world needs at boot — the names, the air, and the
 * dimensions the portals are placed from.
 */
 ...
 */
export function buildCottage(): THREE.Group {
  ...
  // And one in the east wall, which is the only window in the world not facing
  // the sun's own quarter: it takes the morning and is dark by afternoon while
  // the two behind it are still lit. `rotateY(-PI/2)` takes +Z to −X, the
  // inward normal here. The stretch of wall south of the barrel is the one
  // clear run left in the room.
  place(
    root,
    windowBuilder.build({ seed: 4431, curtains: 'none' }),
    halfW - 0.1,
    0,
    1.6,
    -Math.PI / 2,
...
export function buildStore(): THREE.Group {
  ...
  // Both cast: it is the smallest room in the set and the only one with no
  // window, so two flames throwing crate shadows up the walls is the whole of
  // what the place looks like.
  place(root, lantern.build({ seed: 4485, shadows: true }), halfW - 0.7, topOf(box), -1.7, -0.5);
  place(root, candle.build({ seed: 4486, shadows: true }), -1.3, topOf(lid), halfD - 0.5, 0.4);
