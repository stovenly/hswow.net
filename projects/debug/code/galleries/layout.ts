...
import {
  ...
  type ZoneId,
  type ZoneGroup,
} from '@engine/world/Zone';
import { SILENCE, type SoundscapeSpec } from '@engine/audio/Soundscape';
import type { PortalDefinition, PortalEnd } from '@engine/world/Portal';
import type { DoorMaterial } from '@engine/audio/models/door';
import type { MeshBuilder } from '@engine/art/types';
import type { FogVolume } from '@engine/engine/FogVolumes';
import type { GlitchPlacement } from '@engine/engine/Glitch';
import type { HorrorPlacement } from '@engine/engine/Horror';
import { markCollidable } from '@engine/player/Collider';
import { markLabelled } from '@engine/world/Interaction';
import { createRng } from '@engine/art/random';
import { assemble, finish } from '@engine/art/assemble';
import { writing, textSeed } from '@engine/art/writing';
import { DEFAULT_TUNING } from '@engine/player/Controller';
import { flatGround, GRID_TILE } from '@engine/world/floor';

/**
 * Galleries: one room per kind of thing, so the kit can be judged a family at a
 * time. A gallery is a flat floor, a grid to judge scale against, a rank of
 * rows, and a door home. It is a dev room and looks like one — no ground cover,
 * no landforms, no terrain system in any form.
 *
 * **Galleries are silent.** A sound is judged next to the thing making it, and
 * that is true in the *world* and false in here: eight copies of a builder in a
 * line is not a place, so a sound coming out of one has nothing to be judged
 * against. The objects these rooms exist to provide get their emitters where
 * they are actually placed.
 *
 * **Builders are imported rather than read from the registry.** `art/registry`
 * uses `import.meta.glob` and so only exists under Vite, while the headless
 * world check reaches zone definitions through esbuild — and a gallery the
 * check cannot see is a gallery whose portals nobody verifies. Listing them
 * explicitly also makes a gallery a *statement about what belongs together*,
 * which a glob cannot make. The cost is that a new builder does not appear by
 * itself.
 */

/**
 * Seeds per builder. Four is enough to prove a builder varies at all and not
 * enough to see *how* — whether the spread is even or clusters on one shape,
 * and whether a rare variant shows up at the rate it was meant to. Rare things
 * are invisible in small samples, which is when a wrong probability goes
 * unnoticed.
 */
const INSTANCES = 8;
 ...
 *
 * **Distance.** A gallery is for comparing a row along its length, so the
 * arrival has to be far enough back to see the length. Close in, the near
 * instances fill the view and the far end hides behind them.
 *
 * **Facing.** A portal arrival is always in front of its door facing *away*
 * from it, so a door facing +Z puts your back to a rank running toward -Z. The
 * door faces -Z, into the room, which puts the arrival on its near side looking
 * down the rows.
 */
const DOOR_Z = 16;
...
/**
 * Marks on a board that read as writing without being any. The marks themselves
 * are `art/writing`; this is only the sign's own decision about how much text a
 * caption plank carries — a couple of rows, and the third about half the time.
 *
 * A plank rather than real lettering: `art/lettering` earns its keep at sign
 * scale and these are caption scale, so a name on a 26 cm plank would have to
 * be read from a metre away, which is the reach at which the tooltip already
 * does it better. But a blank board is furniture.
 */
...
/**
 * A marker post at the head of a row, carrying the row's name as a tooltip. It
 * stands at eye height with a board on it, so it reads as a label from down the
 * rank rather than from directly above it.
 *
 * The board is blank on purpose — see `scribble`. The board says what a sign
 * *is* and the tooltip says what it *reads*: walk within reach and the name
 * appears over the crosshair, exactly as a door's does.
 *
 * Exported because the sound stage labels its stations the same way.
 */
export function signPost(name: string, display?: string): THREE.Group {
  const group = new THREE.Group();
  group.name = `sign:${name}`;

  // Waist high, at about half eye height. A sign on the horizon is right for a
  // sign you read *instead of* looking at something; this one is a caption on
  // the row behind it, so at eye level it stands directly in front of the thing
  // it names. At the waist it is still an obvious marker from down the rank and
  // still well within the interaction ray.
  const height = DEFAULT_TUNING.eyeHeight * 0.68;
  const post = new THREE.Mesh(new THREE.BoxGeometry(0.09, height, 0.09), SIGN_POST);
  post.position.y = height / 2;
  group.add(post);

  // Raked back a few degrees so it catches the light differently from the posts
  // and the floor. A board dead vertical in a scene lit from above is the
  // darkest thing in view and disappears into its own post.
  //
  ...
  // should. The board is the catalogue and teaches the identifier; the tooltip
  // is the game speaking to the player, and uses the player's word for the
  // thing. See `display` on `MeshBuilder`.
  return markLabelled(group, display ?? signName(name));
}

/**
 * A builder's name as it goes on a sign. The builder names are lowercase
 * identifiers — `bovine`, `small-tree` — and they are what the content data
 * refers to these by, so a sign showing anything else would teach a name that
 * is not the name. Capitalised, hyphens opened to spaces, and that is all.
 */
...
export interface GalleryPlan {
   ...
   * Stated here rather than derived from which hall `propPortals` hangs the
   * door in: that placement is a fact about reachability and this is a fact
   * about kind.
   */
  readonly group?: ZoneGroup;
  readonly builders: readonly MeshBuilder[];
  /**
   * How far apart the instances in a row stand, along -Z. One grid tile unless
   * a gallery says otherwise, which is right for anything you can stand beside
   * and wrong for a room full of buildings — a barn is fourteen metres long, so
   * eight of them four metres apart is one barn. `columns` already spaces the
   * *rows* by what the builders need.
   */
   ...
   * rooms say `iron`, because a hut door standing inside a works gallery is the
   * kit contradicting itself at its own front door.
   */
  ...
  /**
   * Placed fog volumes, in the gallery's own space. Here rather than only on the
   * Fog gallery, because a gallery is a room and any room may want air in it —
   * and threading it through the plan means the one place that turns a plan
   * into a zone stays the one place that knows how.
   */
  readonly fogVolumes?: readonly FogVolume[];
  /**
   * Placed glitch volumes, in the gallery's own space — `fogVolumes`' thread,
   * for its reason.
   */
...
/**
 * Total width of a rank of rows, and where each row starts. Laid out by
 * accumulating radii rather than on a fixed grid, so a hut and a tuft of grass
 * each get the room they need and no more. Centred afterwards, so the door in
 * the middle of the south edge looks down the middle of the rank.
 */
 ...
 * does not hardcode a coordinate the next builder added to the rank would
 ...
 *
 * Sized to the rank and then **clamped**. A flat quad is free to draw and is
 * not free to collide with — see `world/floor.ts` — so the floor is only as big
 * as it has to be and the fog is pulled in to match rather than the other way
 * round. The two numbers have to move together: a floor smaller than the view
 * distance shows its own edge, which is worse than either problem separately.
 */
...
export function galleryZone(plan: GalleryPlan): ZoneDefinition {
  return {
    ...
    environment: {
      ...
      // default is dark and would leave every underside reading as dirt.
...
/**
 * The portal joining a gallery to wherever its door stands in the hub. The hub
 * end is passed in rather than derived, because where the rank of doors stands
 * is the hub's business — a gallery knows what is in it and nothing about the
 * world outside its own door.
 */
