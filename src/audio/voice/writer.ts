 ...
 *
 * A consonant arrives as a feature bundle. Its `manner` picks one of the cores
 * below and everything else in the bundle is a decoration around that core, so
 * a hundred consonants are a dozen gestures and not a hundred cases.
 *
...
function abrupt(c: Consonant): boolean {
  return c.manner === 'glottal' || c.manner === 'click' || c.air === 'ejective';
}
...
function codaTail(c: Consonant): number {
  switch (c.manner) {
    case 'glottal':
      return 0.02;
    case 'trill':
 ...
 *
 * Three terms and they are added: what happens *before* the constriction, the
 * constriction itself, and how long after it the voice waits. This is the one
 * place a mistake shows up as a consonant landing on top of the syllable
 * before it, so each term is named.
 */
...
function coreLead(c: Consonant): number {
  switch (c.manner) {
    ...
    case 'lateralFricative':
      return 0.1;
    case 'nasal':
      return c.voice === 'off' ? 0.1 : 0.07;
    case 'trill':
      return TRILL_PERIOD[c.place] * TRILL_BEATS + 0.01;
    case 'tap':
      return TRILL_PERIOD[c.place] + 0.01;
    case 'click':
      return 0.11;
    case 'breath':
      return 0.065;
...
/** What the release costs: silence after a burst, or a held channel. */
function releaseLead(c: Consonant): number {
 ...
 */
function settle(
  lines: Lines, place: Consonant['place'], vowel: Shape, begin: number, until: number, pos = bodyAt(place),
): void {
  if (inBody(place)) {
 ...
 */
function trill(
  lines: Lines, track: Track, place: Consonant['place'], from: number, beats: number,
): number {
...
function writeOnset(
  ...
  switch (c.manner) {
    ...
    case 'lateralFricative':
      ...
      return;
    case 'nasal':
      writeNasal(lines, c, on, begin, level, vowel, me);
      return;
    case 'trill':
    case 'tap':
    ...
    case 'lateral':
    case 'approximant':
      writeLiquid(lines, c, on, begin, level, vowel);
      return;
    case 'click':
...
/**
 * Still, shut, held, let go: the closing articulator and nothing else. The
 * closure is silent and the tube is being pushed into, so the release makes
 * the burst by itself.
 *
 * The airstream and the attack ride on the same gesture. An ejective shuts the
 * folds as well and lets the voice in late; an implosive drops the larynx
 * behind the closure (the dip is on the f0 line) and lets go with no puff at
 * all; a prenasal hums through the nose first and seals it before the burst.
 */
function writeStop(
  lines: Lines, c: Consonant, on: number, begin: number, level: number, vowel: Shape, me: Identity,
): void {
  const eject = c.air === 'ejective';
  const implode = c.air === 'implosive';
...
/**
 * A narrow channel with no voice behind it. The tube hisses because it is
 * narrow, not because a filter was told to — and narrower than any vowel gets,
...
function writeFricative(
  ...
  if (inBody(c.place)) {
    // The body is what makes the channel, so it must not drift there early.
    lines.hold('bodyPos', begin - 0.03);
    lines.at('bodyPos', begin, g.bodyPos ?? vowel.bodyPos, 'lin');
    lines.hold('bodyPos', on - 0.02);
  } else {
    lines.at('bodyPos', begin, g.bodyPos ?? vowel.bodyPos, 'lin');
  }
  if (g.track !== 'bodyDia') lines.at('bodyDia', begin, g.bodyDia ?? vowel.bodyDia, 'lin');
  if (g.track !== 'lips') lines.at('lips', begin, g.lips ?? vowel.lips, 'lin');

  const back = g.track === 'tip' ? TIP_OPEN : g.track === 'lips' ? vowel.lips : vowel.bodyDia;
  lines.hold(g.track, begin - 0.03);
  lines.at(g.track, begin + 0.012, g.gap, 'lin');
...
/**
 * The tongue up in an l with air forced past its sides: voiced for a moment,
 * then the voice goes and it hisses through the gap.
 */
function writeLateralFricative(
  lines: Lines, on: number, begin: number, level: number, vowel: Shape, me: Identity,
): void {
...
/**
 * A hum through a shut mouth that opens into the vowel. The nose is what makes
 * the sound; the mouth is closed the whole time and lets go fast. With the
 * folds off it is the same gesture on breath alone, and the voice comes on
 * just before the mouth opens.
 */
function writeNasal(
  lines: Lines, c: Consonant, on: number, begin: number, level: number, vowel: Shape, me: Identity,
): void {
  ...
  if (inBody(c.place)) settle(lines, c.place, vowel, begin, on);
  if (c.voice === 'off') {
...
/**
 * A real constriction with the voice on, made, held into the vowel and let go
 * slowly. It sits right on the hiss threshold: as tight as the tube allows
 * without noise, which is what a liquid is.
 */
function writeLiquid(
  lines: Lines, c: Consonant, on: number, begin: number, level: number, vowel: Shape,
): void {
  const g = liquidGap(c, vowel);
  const { track, open } = closer(c.place, vowel);
  lines.at('jaw', begin, g.jaw, 'lin');
  if (inBody(c.place)) {
    lines.hold('bodyPos', begin - 0.03);
...
/**
 * Two closures: the tongue body all but seals the velum while the front shuts,
 * the front snaps open into the pocket between them, then the back lets go. No
 * voice and no breath — the pop is all there is.
 */
function writeClick(
  lines: Lines, c: Consonant, on: number, begin: number, level: number, vowel: Shape,
): void {
...
/**
 * An h: the vowel's own shape, breathed before it is voiced. In the throat,
...
/**
 * Nothing in the mouth: the folds shut, and open again onto the vowel with a
 * catch.
 */
function writeGlottal(lines: Lines, on: number, begin: number, level: number, vowel: Shape): void {
  lines.at('jaw', begin, vowel.jaw, 'lin');
  lines.at('bodyPos', begin, vowel.bodyPos, 'lin');
  lines.at('bodyDia', begin, vowel.bodyDia, 'lin');
  lines.at('lips', begin, vowel.lips, 'lin');
...
function writeCoda(
  ...
  switch (c.manner) {
    ...
    case 'tap': {
      ...
      const done = trill(lines, track, place, end - 0.01, c.manner === 'tap' ? 1 : 2);
    ...
    case 'lateralFricative': {
      ...
      const gap = place === 'ridge' ? LATERAL : place === 'throat' ? 0.24 : fricGap(c).gap;
