...
// A crag: a broken tor of stone four to seven metres high — the accent a run of
// rock needs, so the line reads as terrain rather than a wall.
//
// Three to five blocks piled up, each turned well round on the one below, with the
// joints running horizontally: a single tapered mass at this height is a cartoon
// spire. The blocks lean cumulatively, so the column has one overall list. Tops
// and bottoms are flat, because a bedding plane is the plane the rock split along
// — two ragged faces have nothing to meet on and drive corners out through sides.
// Two or three shed blocks at the foot widen the base.
export const crag: MeshBuilder = {
  ...
  build({ seed = 1, scale = 1 } = {}) {
    ...
    // The list of the whole column, and small: the blocks meet on flat faces, so
    // every degree of tilt is a wedge of daylight at one edge of the joint and a
    // corner coming through at the other.
    ...
    for (let i = 0; i < blocks; i++) {
      // Aspect is rolled wide on purpose: a course half as tall as it is broad is a
      // shelf, one twice as tall is a wedge, and a tor has both.
      ...
      bedding = y + half;
      // Each block sits off the middle of the one below, by enough to read and not
      // enough to overhang — a block that steps further than it has stone to spare
      // has a corner in mid-air.
