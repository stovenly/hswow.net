...
// A stand of nettles: straight stems in opposed leaf pairs. Upright and regular
// is the silhouette, which is unusual here — nearly every other plant is
// identified by how it leans — and that mechanical repetition is what tells it
// apart from a wildflower clump at ten metres.
export const nettle: MeshBuilder = {
  ...
  build({ seed = 1, scale = 1 } = {}) {
    ...
    // A nettle bed is dense, but a clump built at real density is a solid green mass
    // with no stalks visible in it, and the upright repeating stems are the entire
    // silhouette. Density comes from placing several clumps.
    ...
    for (let s = 0; s < stalks; s++) {
      ...
      for (let t = 1; t <= tiers; t++) {
        ...
        // Shrinking hard toward the top: a real nettle grades steeply, and by the
        // growing tip the leaves are barely more than bracts. It also gives the plant
        // a point, which is most of what says nettle in a silhouette.
        const tier = height * rng.range(0.1, 0.16) * (1 - (t / tiers) * 0.72);
        for (const side of [-1, 1]) {
          // A per-leaf size rather than a per-pair one: two blades built to identical
          // dimensions occasionally land exactly on each other, and a few percent of
          // variation makes that impossible rather than merely unlikely.
      ...
      // --- the crown ----------------------------------------------------------
      // A tight rosette of very small leaves at the growing tip. A nettle does not
      // stop at its last full-sized pair; without this the stalk ends on a pair of
      // leaves and a bare centimetre of stem, which reads as cut.
      ...
      if (rng.chance(0.6)) {
        for (const side of [-1, 1]) {
          ...
          // ±0.8 rad differs from its opposite by 1.6, which is within two hundredths
          // of the π/2 that would make the two identical.
