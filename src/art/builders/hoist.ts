...
// A gantry beam with a chain hoist hanging off it. The dangling chain is the
// silhouette: a beam on legs is a doorway, and a beam with a line falling out of
// the middle of it is a crane. The chain is a stack of short links in two
// orientations turned a quarter from each other, which costs less than the hook.
export const hoist: MeshBuilder = {
  ...
  build({ seed = 1, scale = 1 } = {}) {
    ...
    // --- the runway ----------------------------------------------------------
    // An I-beam as three boxes, two flanges and a web: the flanges are what make it
    // read as rolled steel rather than as a length of timber.
    const beamY = height;
    for (const [y, w, t] of [
      [beamY + 0.11, 0.3, 0.05],
      [beamY - 0.11, 0.3, 0.05],
    ...
    for (const side of [-1, 1]) {
      ...
      // A knee brace from the leg inward and up to the beam, spanning two known
      // points so it lands on both. A brace triangulates the corner between a leg
      // and the beam it carries, so it has to run across that corner.
    ...
    if (rng.chance(0.72)) {
      ...
      // Spaced closer than a link is long, so consecutive rings interlock: the pitch
      // is a little under the ring's outer diameter, and every link passes through
      // the two either side of it. At exactly a link's length it is a row of rings
      // touching end to end.
      const ring = 0.035;
      const tube = 0.011;
      ...
      // The chain stops at the hook; the hook does not climb into the chain. The
      // hook's geometry is solved from the bottom up and the chain is given what is
      // left. The one deliberate overlap is the last link and the top of the eye,
      // which is how a chain is actually attached to a hook.
      const bend = 0.075;
      ...
      for (let i = 0; i < links; i++) {
        ...
        // Both orientations vertical: a torus is built in the XY plane, which is
        // already right for a hanging link, and alternating about Y is the whole
        // trick — a stack all facing one way is a strip of washers.
        link.rotateY(i % 2 === 0 ? 0 : Math.PI / 2);
        link.translate(carriage, y, 0);
        parts.push({ geometry: link, color: shade(iron, 0.92), sway: 0 });
      }

      // The hook: a shank, and a curl swept from rods between named points. Points
      // on a circle joined end to end cannot come apart, because each segment starts
